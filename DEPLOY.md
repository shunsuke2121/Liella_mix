...
```
liella-mixer/
  index.html      ポータル（トップ・メンバー図鑑・誕生日カウントダウン・各アプリへの入口）
  mixer.html      ボーカル重ねミキサー（旧 index.html）
  quiz.html       声当てクイズ
  compare.html    聴き比べ
  data.js         全ページ共通データ（MEMBERS/SONGS/GROUPS・色ユーティリティ）★編集はここに集約
  audio/*.m4a     圧縮音源（AAC 160k・各約4MB）
  img/*.png       立ち絵・アイコン・ジャケ写
  middleware.js   Basic認証ゲート（音源含む全体を保護）
  vercel.json     音源のキャッシュ設定
  start.bat       ローカル起動用（デプロイには含めない）
```
※ メンバーや曲を追加・修正するときは **`data.js` だけ**を直せば全ページに反映される。

## ページ
- **index.html（ポータル）** … 次の誕生日カウントダウン（メンバーカラーで色づく）、メンバー図鑑（期別・ユニット・誕生日順で絞り込み／カード→詳細シート→そのメンバーでミキサーを開く）、各アプリ導線、公式リンク集。
- **mixer.html** … 従来のミキサー（重ね・タイムライン・設定タブ、ジャケ写テーマ、Media Session）。
- **quiz.html（声当てクイズ）** … ランダムな曲・メンバー・再生位置から声だけを再生して当てる。4択/全員、3・6・10秒、ヒント（候補を半減）、連続正解と最高記録を `localStorage`(`liella_quiz`) に保存。イントロ/アウトロを避けた区間を出題し、前後をフェードして切れ目を自然に。
- **compare.html（聴き比べ）** … 全員を同期再生し、選択した1人だけを可聴に（切替は瞬時）。未読込のメンバーは押した時点で読み込み→届き次第に鳴る。A/B区間リピート、オケ音量つまみ、キーボード操作。
...
- **ジャケ写でUI全体の色が変わる**：カバー画像から鮮やかな代表色を抽出（canvasで24pxに縮小→彩度×明度スコア最大の色）。`--theme`（背景ティント）/`--themeSoft`（NPカード）/`--accent`（操作色＝ホバー・枠・スライダーの accent-color）/`--grad`（タブ・再生ボタン・選択チップのグラデ）を上書き。グラデ相方は**色相をピンク(320°)方向へ最大55°寄せる**ので、オレンジ曲＝橙→桃、青＝青→紫とポップにまとまる。曲切替で0.5sフェード。ジャケ無し曲はブランド青。実装：`applyCoverTheme`/`setTheme`/`rgb2hsl`/`hsl2rgb`（同一オリジンなのでcanvasは非汚染）。
- **歌っている声でアイコンが光る**：各メンバーの `gainNode` に AnalyserNode を接続し、rAF（`tick`→`updateLevels`）で RMS を算出→`--lvl`(0〜1) をアイコンに設定。`.av.on .ring` が `scale()` と発光リングで反応。停止時は0にリセット。無効メンバーはgain0なので常に0。
- **おまかせ／共有**：構成タブに「🎲 シャッフル」（2〜4人ランダム）「✌️ デュエット」（2人）「🔗 共有リンク」。共有リンクは `#s=<曲id>&m=<slug,slug>` をクリップボードにコピーし、開くと**その曲＋構成で復元**（`applyHashConfig`、`hashchange`にも追従）。操作結果はトーストで通知。
- **キーボードショートカット**：`Space`再生/停止、`1〜9`(`0`,`-`)メンバー切替、`A`全員、`I`オケのみ、`S`シャッフル、`←→`5秒移動。入力欄にフォーカス中は無効。
